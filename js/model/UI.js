...
export class Text {
  constructor(text, x, y, textAlign, color = '#eeeeee') {
    ...
    this.height = 32;
    // this.font = `${
    //     this.height}px Courier, Helvetica, Arial, LiHei Pro, 黑體-繁,
    //     微軟正黑體, sans-serif`;
...
export class TextManager {
  constructor(canvas) {
    ...
    this.textList = [];
    this.newTextY = 0;
    this.newTextY = this.canvas.height;
  }
  addText(text, color) {
    const textObject =
...
export class BossBar extends UI {
  ...
  render(ctx, canvas) {
    ...
    ctx.fillRect(
        ...
        Math.max(this.barLength * (this.target.health / this.target.maxHealth), 0),
        this.barHeight);
